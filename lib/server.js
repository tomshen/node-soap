...
Server.prototype._executeMethod = function(options, callback) {
  ...
  function handleResult(result) {
    ...
    if (result.hasOwnProperty('Fault')) {
      body = '<soap:Fault>' +
      '<faultcode>' + result.Fault.faultcode + '</faultcode>' +
      '<faultstring>' + result.Fault.faultstring + '</faultstring>' +
      '</soap:Fault>'
    }
    else if (style === 'rpc') {
  ...
  var result = method(args, handleResult, header);
